...
class Excel5 {
    ...
    /**
     * Use OLE reader to extract the relevant src streams from the OLE file
     *
     * @param string $file
     */
    ...
    public function parseWorksheetInfo() {
        if ($this->sheets === null) {
            // total byte size of Excel src (workbook global substream + sheet substreams)
    ...
    private function readBof() {
        ...
        // offset: 2; size: 2; type of the following src
    ...
    private function readDefault() {
        $length = Format::getUInt2d($this->data, $this->pos + 2);
        //$recordData = $this->readRecordData($this->src, $this->pos + 4, $length);
    ...
    /**
     * Read record src from stream, decrypting as required
     *
     * @param string $data Data stream to read from
     * @param int $pos Position to start reading from
     * @param int $len Record src length
     *
     * @throws ParserException
     * @return string Record src
     */
    private function readRecordData($data, $pos, $len) {
        ...
        if ($this->encryption == self::MS_BIFF_CRYPTO_RC4) {
            ...
            // Decrypt record src (re-keying at the end of every block)
     ...
     * @var string $salt_data       Salt src
     * @var string $hashedsalt_data Hashed salt src
    ...
    private function readSst() {
        // offset within (spliced) record src
        $pos = 0;

        // get spliced record src
        ...
        for ($i = 0; $i < $nm; ++$i) {
            ...
            if ($hasAsian) {
                // For Asian phonetic settings, we skip the extended string src
    ...
    /**
     * Reads a record from current position in src stream and continues reading src as long as CONTINUE records
     * are found. Splices the record src pieces and returns the combined string as if record src is in one piece.
     * Moves to next current position in src stream to start of next record different from a CONtINUE record
     *
     * @return array
     */
    private function getSplicedRecordData() {
        ...
        do {
            ...
            //$identifier = Cell::getInt2d($this->src, $this->pos);
    ...
    private function readByteStringShort($subData) {
        ...
        // size in bytes of src structure
